        ...
        <div class="card mb-3">
          ...
          <div class="card-body">
            <div className="form-group">
              <div className="form-row">
                <div className="col-md-6">
                  <div className="form-group">
                      <label>Fltrar por nombre*</label>
                      <input type="text" name="name" class="form-control" v-model="task_name_idv">
                        <span v-for="error in errors.name" class="text-danger">{{ error }}</span>
                  </div>
                </div>
                <div className="col-md-6">
                  <label> Estado *</label>
                  <multiselect v-model="task_status_idv" :options="statuses" placeholder="Seleccione un Estado" label="name" track-by="name"></multiselect>
            ...
            <div className="form-group">
              <div class="form-row">
                <div class="col">
                  <button type="button" style="width: 100%" class="btn btn-primary" v-on:click.prevent="searchByIndividual(task_name_idv, task_status_idv)">Buscar</button>
                </div>
                <div class="col">
                  <button type="button" style="width: 100%" class="btn btn-secondary" v-on:click.prevent="task_name_idv='';task_status_idv='';">Cancelar</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="card mb-3">
          ...
          <div class="card-body">
            <div className="form-group">
              <div className="form-row">
                <div className="col-md-6">
                  <div className="form-group">
                      <label>Fltrar por nombre*</label>
                      <input type="text" name="name" class="form-control" v-model="task_name_col">
                      <span v-for="error in errors.name" class="text-danger">{{ error }}</span>
                  </div>
                </div>
                <div className="col-md-6">
                  <label>Estado *</label>
                  <multiselect v-model="task_status_col" :options="statuses" placeholder="Seleccione un Estado" label="name" track-by="name"></multiselect>
                  <span v-for="error in errors.teaching_periods_id" class="text-danger">{{ error }}</span>
                </div>
              </div>
            </div>
            ...
            <div className="form-group">
              <div class="form-row">
                <div class="col">
                  <button type="button" style="width: 100%" class="btn btn-primary" v-on:click.prevent="searchByColective(task_name_col, task_status_col)">Buscar</button>
                </div>
                <div class="col">
                  <button type="button" style="width: 100%" class="btn btn-secondary" v-on:click.prevent="task_name_col='';task_status_col='';">Cancelar</button>
                </div>
              </div>
            </div>
          </div>
            ...
            <table class="table table-striped table-bordered">
              ...
              <tbody>
                <tr v-for="(value, index) in tasks_contents">
                  ...
                  <td>{{ value.status == 'finish' ? 'Finalizada' : 'Activa' }}</td>
                  <td>{{ value.tasks.type }}</td>
                  <td>{{ value.teachers_id  ? 'Si' : 'No'}}</td>
                  <td>No</td>
                </tr>
              </tbody>
            </table>
...
<script>
  ...
  export default { 
    ...
    methods: {
      ...
      getAll(){
        console.log('todo');
        var url = '/dashboards/report/list';
      ...
      searchByIndividual(task_name, status){
        let vm = this;
        axios.get('/dashboards/manage/indivial/' + (task_name ? task_name : 'none') + '/' + (status ? status.enum : 'none')).then(function (response) {
      ...
      searchByColective(task_name, status){
        let vm = this;
        axios.get('/dashboards/manage/colective/' + (task_name ? task_name : 'none') + '/' + (status ? status.enum : 'none')).then(function (response) {
          console.log("hello2", response.data);
          vm.tasks_contents =  response.data.tasks.data;
          vm.pagination = response.data.pagination;
        });
      ...
      show(value){
        ...
        var array = [];
        if(value.tgxts != ""){
          $.each(value.tgxts, function(key, value){
        ...
        this.find = {
          ...
          'teachers_group': value.tgxts != "" ? array : "No Data",
      ...
      getTeaching_period(name){
        var url = '/dashboards/teaching_periods/' + name;
        axios.get(url).then(response => {
