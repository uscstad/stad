  ...
  <div class="containerx">
    <div class="card">
      <div class="card-header">
        <div class="col-md-12">
          <i class="fas fa-clipboard-list"></i> <b>Actividades</b>
        </div>
      </div>
      <div class="card-body">
        <div class="col-sm-12">
          <div class="row">
            <div class="col-12 col-sm-6">
              <div class="form-group">
                <input type="text" class="form-control" v-model="search" placeholder="Buscar Actividades" />
              </div>
            </div>
            <div class="col-12 col-sm-6 text-right">
              <a href="./" class="btn btn-primary"><i class="fas fa-chevron-circle-left fa-lg"></i> Atras</a>  
              <a href="#" class="btn btn-primary" @click="showCreate=true"><i class="fas fa-plus-circle fa-lg"></i> Actividades</a>
            </div>
          </div>
        </div>
        ...
        <div class="col-md-12" v-else>
          <h4>No se han encontrado registros de las Actividades.</h4>
        </div>
      </div>
            ...
            <div class="modal-dialog modal-xl modal-dialog-centered">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title"><i class="fas fa-calendar-alt"></i> <b>Creación de Actividad</b></h4>
        ...
        <div class="modal-mask">
          <div class="modal-wrapper">
            <div class="modal-dialog modal-xl modal-dialog-centered">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title"><i class="fas fa-calendar-alt"></i> <b>Edición de Actividad</b></h4>
                  <button type="button" class="close" @click="cancel">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
        ...
        <div class="modal-mask">
          <div class="modal-wrapper">
            <div class="modal-dialog modal-xl modal-dialog-centered">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title"><i class="fas fa-calendar-alt"></i> <b>Eliminar Actividad</b></h4>
                  <button type="button" class="close" @click="cancel">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
                <div class="modal-body" style="margin: 0px 0;max-height: 400px;overflow-y: auto;">
                  <h4 class="text-center">¿Esta seguro de que desea eliminar a <strong>{{find.name}} </strong> de las Actividades?</h4>
                </div>
                <div class="modal-footer">
...
<script>
  ...
  export default { 
    ...
    methods: {
      ...
      getAll(){
        var url = 'activities/list';
        this.fetchStories(url);
      },
      fetchStories(page_url) {
        let vm = this;
        axios.get(page_url).then(function (response) {
          vm.searchData = response.data.activities;
      ...
      store(){
        ...
        axios.post(url,{
          ...
          toastr.success('Nueva actividad creada correctamente');
        }).catch(error => {
      ...
      update(id){
        var url = 'activities/' + id;
        axios.put(url, this.find).then(response => {
          this.getAll();
          this.find = {'id': '', 'name': '', 'description': ''};
          this.errors = [];
          this.showUpdate = false;
          toastr.warning('Actividad actualizada correctamente');
        }).catch(error => {
          this.errors = error.response.data.errors;
        });
      },
      ...
      deleteOne(value){
        ...
        var url = 'activities/' + value.id;
        axios.delete(url).then(response => {
          this.getAll();
          toastr.info('Actividad eliminada correctamente');
        });
      },
      cancel(){
        this.name = '';
